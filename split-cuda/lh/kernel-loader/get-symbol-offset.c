...
off_t getSymbolOffset(char *pathname, char *symbol) {
  ...
  for (i = 0; i < elf_hdr.e_shnum; i++) {
    ...
    if (sect_hdr.sh_type == SHT_SYMTAB) {
      ...
      if (access("/lib/debug/.build-id", F_OK) == 0) { // If Debian/Ubuntu
        ...
        assert(fd != -1);
        return getSymbolOffset(debugLibName, symbol);
      }
      free(debugName);
      foundDebugLib = 1;
    }
...
#ifdef STANDALONE
int main() {
  off_t mmap_offset = getSymbolOffset("/lib64/ld-linux-x86-64.so.2", "mmap");
  if (! mmap_offset) mmap_offset =
         getSymbolOffset("/usr/lib/debug/lib64/ld-linux-x86-64.so.2", "mmap");
  if (mmap_offset == 0) {
