...
safeLoadLib(const char *name)
{
  ...
  if (ld_so_fd == -1) {
        fprintf(stderr, "Error opening ELF interpreter %s: %s\n", elf_interpreter, strerror(errno));
        exit(EXIT_FAILURE);
    }
  assert(ld_so_fd != -1);
...
//	  exit(EXIT_FAILURE);
 ...
 // sbrk_offset = get_symbol_offset(debug_ld_so_fd, buf, "sbrk");
  ...
  if (mmap_offset == -1) {
        fprintf(stderr, "Error getting mmap offset for %s\n", name);
        exit(EXIT_FAILURE);
    }

  sbrk_offset = get_symbol_offset(ld_so_fd, name, "sbrk");
if (sbrk_offset == -1) {
        fprintf(stderr, "Error getting sbrk offset for %s\n", name);
        exit(EXIT_FAILURE);
    }
