...
extern "C"{
off_t getSymbolOffset(char *pathame, const char *symbol);
...
static void* createNewHeapForRtld();
...
int main(int argc, char *argv[], char *envp[]) {
  ...
  for (i = 1; i < argc; i++) {
    if (strcmp(argv[i], "-h") == 0) {
      fprintf(stderr, "USAGE: UH_PRELOAD=/path/to/libupperhalfwrappers.so %s [-a load_address] <command_line>\n", argv[0]);
      ...
      i++;
        if (argc < 3){
	  fprintf(stderr, "USAGE: %s --restore /path/to/ckpt.img \n",argv[0]);
	  return -1;
        }
	int ckptFd = atoi(argv[2]);
// Open the checkpoint file
    //int ckptFd = open(argv[2], O_RDONLY);
    //if (ckptFd == -1) {
	...
	if (rc < 0){
	  fprintf(stderr, "Failed to setup lhinfo for the upper half. Exiting...\n");
	}
	/*
	 * restoreCheckpoint will
	 * 1. read the MtcpHeader
	 * 2. restore the memory region of the application from the ckpt image.
  ...
  char *interp_base_address =
    static_cast<char*>(load_elf_interpreter(ld_so_fd, elf_interpreter, ld_so_addr, &ld_so_elf_hdr));
...
size_t len = stack.size;
  ...
  // FIXME:
  //   Should add check that interp_base_address + 0x25405000 not already mapped
  //   Or else, could use newer MAP_FIXED_NOREPLACE in mmap of deepCopyStack
 // char* avail_add = findAvailableAddress();
  char *dest_stack = deepCopyStack(argc, argv, (char *)&argc, (char *)&argv[0],
                                   cmd_argc+1, cmd_argv2, interp_base_address + 0x25405000 ,&auxv_ptr,len);
  ...
  if (!newHeap) {
    fprintf(stderr, "Error creating new heap for RTLD. Exiting..\n");
    exit(-1);
  }

  off_t mmap_offset = getSymbolOffset(elf_interpreter, "mmap");
  if (! mmap_offset) {
    ...
    mmap_offset = getSymbolOffset(buf, "mmap"); // elf interpreter debug path
  }
  assert(mmap_offset);
#ifdef DEBUG
  fprintf(stderr,
  ...
  off_t sbrk_offset = getSymbolOffset(elf_interpreter, "sbrk");
  if (! sbrk_offset) {
    char buf[256] = "/usr/lib/debug";
    buf[sizeof(buf)-1] = '\0';
    ssize_t rc = 0;
    ...
    sbrk_offset = getSymbolOffset(buf, "sbrk"); // elf interpreter debug path
  }
  assert(sbrk_offset);
#ifdef DEBUG
  fprintf(stderr,
  ...
  void* (*sbrk_wrapper_ptr)(intptr_t) = &sbrkWrapper;
  void* (*mmap_wrapper_ptr)(void*, size_t, int, int, int, off_t) = &mmapWrapper;
  patch_trampoline(interp_base_address + sbrk_offset, reinterpret_cast<void*>(sbrk_wrapper_ptr));
  patch_trampoline(interp_base_address + mmap_offset, reinterpret_cast<void*>(mmap_wrapper_ptr));
 ...
 int rc = setupLowerHalfInfo();
 if(rc<0){
	 fprintf(stderr, "Failed to set up lhinfo for the upper half");
 }
...
char * map_elf_interpreter_load_segment(int fd, Elf64_Phdr phdr,
  ...
  if (first_time && ld_so_addr == NULL) {
    rc2 = mmapWrapper((void *)addr, len, prot, MAP_PRIVATE, fd, offset);
  } else {
    assert((char *)addr+len >=
           (char *)base_address + phdr.p_vaddr + phdr.p_memsz);
    rc2 = mmapWrapper((void *)addr, len, prot, MAP_PRIVATE|MAP_FIXED, fd, offset);
  }
  if (rc2 == MAP_FAILED) {
    perror("mmap"); exit(1);
  }
  ...
  if (first_time) {
    first_time = 0;
    base_address = static_cast<char*>(rc2);
  }
  return base_address;
}
...
setupLowerHalfInfo()
{
  ...
  if (syscall(SYS_arch_prctl, ARCH_GET_FS, &lhInfo.lhFsAddr) < 0) {
    fprintf(stderr, "Could not retrieve lower half's fs.");
    return -1;
  }
  ...
  if (rc < 0) {
    fprintf(stderr, "Error writing address of lhinfo to file. Exiting...\n");
    return -1;
  }
  return 0;
}
...
writeLhInfoToFile()
{
  ...
  if (fd < 0) {
    fprintf(stderr, "Could not create addr.bin file");
    return -1;
  }

  rc = write(fd, &lhInfo, sizeof(lhInfo));
  if (rc < sizeof(lhInfo)) {
    fprintf(stderr, "Wrote fewer bytes than expected to addr.bin.");
    rc = -1;
  }
  close(fd);
  return rc;
