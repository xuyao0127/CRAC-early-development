...
#include "get-symbol-offset.h"
#include "copy-stack.h"
...
RestoreTarget* get_restore_target_info(DmtcpRestart &dmtcpRestart);
...
int main(int argc, char *argv[], char *envp[]) {
  ...
  if (restore_mode) {
    ...
    RestoreTarget *t = get_restore_target_info(dmtcpRestart);
 ...
 *
 * This function checks if the environment variable "CRAC_FIRST_TIME" is set. If not,
...
void set_addr_no_randomize(char *argv[]) {
  extern char **environ;
  const char *first_time = "CRAC_FIRST_TIME";
 ...
 *  file name formatted `crac_tmp_lh_info_[hostname]_[pid]`.  
 * Launch Mode:
...
void write_lh_info_addr(int restore_mode) 
{
    ...
    setenv("CRAC_LH_INFO_ADDR", lh_info_addr_str, 1);
  }
}
                      ...
                      char *interp_base_address, Elf64_auxv_t **auxv_ptr, void **stack_bottom)
{
  // Get stack's size limit. We need to leave enough space between the loader
    ...
    lh_info->uh_stack_start = (char*) ROUND_DOWN(interp_base_address + LOADER_SIZE_LIMIT);
    lh_info->uh_stack_end = (char*) ROUND_UP(*stack_bottom);
